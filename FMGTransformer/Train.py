...
# 先读取数据
data = pd.read_csv("../data/cuihua1140.csv")
...
X_categ = data[categorical_features].values  # 需要分类的特征
X_cont = data[continuous_features].values    #连续的特征
...
class CustomTabularDataset(Dataset):
    def __init__(self, X_categ, X_cont, y):
        self.X_categ = torch.tensor(X_categ, dtype=torch.long)  # 分类特征的输入需要是long类型
        self.X_cont = torch.tensor(X_cont, dtype=torch.float)  # 连续特征的输入是float类型
...
model = FMGTransformer(
    categories=categories,             # 分类特征每个类别的数量
    num_continuous=num_continuous,      # 连续特征的数量
    dim=128,                            # 嵌入维度
    depth=2,                           # Transformer的层数   #原来是4  我改了一下6，效果不好记得要改回来！！！
    heads=4,                           # Multi-head Attention的头数
    dim_head=32,                       # 每个注意力头的维度    注意力头的维数应该怎么选
    dim_out=1,                         # 输出维度（回归问题输出1个值）
    num_special_tokens=2,              # 特殊的token数量 (通常为2)
    attn_dropout=0.1,                   # 注意力层的dropout
    ff_dropout=0.1,                      # Feed-forward层的dropout
)
...
def train_model(model, train_loader, test_loader,criterion, optimizer, scheduler,epochs=250,flag = None):
    ...
    train_losses = []  # 保存训练集的 Loss
    train_r2_scores = []  # 保存训练集的 R²
    ...
    if flag == None :
        plot_loss_and_r2_curves(train_losses=train_losses,
                                ...
                                epochs=epochs,  # 根据你设置的epoch数填
                                save_path=None  # 或者填一个保存路径，如 "C:/Users/你/路径")
                                )
        #测试集
        model.eval()
        ...
        metrics = calculate_metrics(all_test_labels, all_test_preds)  # 这是反归一化前的指标
        print(metrics)
